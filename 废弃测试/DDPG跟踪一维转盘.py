...
def rotation_limit_range(angle):
    if angle>np.pi:
        angle-=2*np.pi
    elif angle<=-np.pi:
        angle+=2*np.pi
...
class CarEnv(gym.Env):
    ...
    def reset(self):
        ...
        self.angle=0.0
        self.omega=0.0
        self.target_angle = np.clip(np.random.randn(), -np.pi/4, np.pi/4)  # 初始位置
        self.target_omega = np.clip(np.random.randn(), -1, 1)  # 初始位置
        self.state = [self.angle, self.target_angle]
        self.done = False
        return self.state

    def step(self, action):
        self.angle = self.state[0]  # 从数组中提取标量
        self.target_angle = self.state[1]  # 从数组中提取标量
        # 目标运动
        self.target_angle += self.target_omega * dt
        self.target_angle = rotation_limit_range(self.target_angle)
        # 控制角速度方式
        omega = action[0]
        self.angle += omega*dt
        self.angle=rotation_limit_range(self.angle)

        self.state = [self.angle, self.target_angle]
        self.t += dt
        # 定义奖励函数
        reward = -abs(self.target_angle-self.angle)  # 奖励与位置偏差的绝对值成反比
        # 判断是否结束
        self.done = self.t > 8
    ...
    with tqdm(total=int(num_episodes/10), desc='Iteration %d' % i) as pbar: # 进度条
        for i_episode in range(int(num_episodes/10)): # 每个1/10的训练轮次
            ...
            while not done: # 每个训练回合
                ...
                # 把最后一个训练轮次当做测试轮
                if (i*10+i_episode+1)==int(num_episodes/10)-1:
...
# 绘制第三个图形
plt.figure()
plt.plot(range(length), trajectory[:,0], label='Car Trajectory')  # 轨迹
# plt.axhline(y=0, color='r', linestyle='--', label='Target Line')  # 目标直线
plt.plot(range(length), trajectory[:,1], label='target Trajectory')  # 目标轨迹
plt.xlabel('Frap')
plt.ylabel('Location')
