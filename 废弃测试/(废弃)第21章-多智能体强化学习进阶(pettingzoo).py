...
# 替换的部分
# env = simple_adversary_v3.env(render_mode="human") # human直接显示、rgb_array返回numpy数组方便做视频处理或图像分析、ansi返回字符串,用的最少
...
action_dims = []
# 原版为列表，被改成字典之后写法有变
for action_space in env.action_spaces.values(): # 字典可以遍历dict.keys()或者dict.values()
...
for i_episode in range(num_episodes):
    ...
    for e_i in range(episode_length):
        ...
        if replay_buffer.size()\
            ...
            # 更新网络参数
            for a_i in range(len(env.agents)): # a_i对应agents
