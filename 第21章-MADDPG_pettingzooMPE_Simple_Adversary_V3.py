# 假设有N个智能体，那么用到的网络数量是4N，2N个actor和2N个critic
# 如果critic不是集中式的，那么CTDE要怎么体现出来？ 通过训练阶段输入给每个智能体的critic全局的状态和观测信息来体现。
...
class MADDPG:
    ...
    def update(self, sample, agent_idx):
        ...
        # 这里的*next_states和*target_action表示将next_states和target_action这两个可迭代对象中的元素解包，
        # 并将它们作为独立的参数传递给torch.cat函数。torch.cat函数是PyTorch库中的一个函数，用于连接张量（tensors）。
        # 如果next_states是[A, B, C]，target_action是[X, Y]，那么torch.cat((*next_states, *target_action), dim=1)等同于torch.cat([A, B, C, X, Y], dim=1)，
        # 这将把A、B、C、X和Y这些张量在第1维上进行连接。
        # todo 不同智能体之间的critic有没有通道相互连接？
