...
# ID embedding dimension for concatenation
ID_EMB_DIM = 16
...
class ParallelAttentionActor(nn.Module):
    def __init__(self, n_agents, node_dim, hidden_dim, id_dim, action_dim):
        super().__init__()
        self.n = n_agents
        self.hidden_dim = hidden_dim
        self.id_dim = id_dim # 专门给 ID 的维度，建议 16 或 32
        
        # 总的嵌入维度（必须能被 num_heads 整除）
        self.total_dim = hidden_dim + id_dim 
        
        # 1. 物理特征映射 (只映射到 hidden_dim)
        self.proj_self = nn.Linear(4, hidden_dim)
        self.proj_lm = nn.Linear(2, hidden_dim)
        self.proj_other = nn.Linear(2, hidden_dim)
        
        # 2. 身份标签 (只映射到 id_dim)
        self.agent_id_emb = nn.Embedding(n_agents, id_dim)
        self.lm_id_emb = nn.Embedding(n_agents, id_dim)
        ...
        # 3. 注意力模块 (输入维度是 total_dim)
        self.attn_lm = nn.MultiheadAttention(embed_dim=self.total_dim, num_heads=2, batch_first=True)
        self.attn_other = nn.MultiheadAttention(embed_dim=self.total_dim, num_heads=2, batch_first=True)
        
        # 4. 融合决策层
        self.fusion = nn.Sequential(
            nn.Linear(self.total_dim * 3, hidden_dim),
            nn.ReLU(),
    ...
    def forward(self, obs, agent_id, active_mask=None):
        ...
        f_self = self.proj_self(obs[:, 0:4]).unsqueeze(1) # (B, 1, H)
        f_lm = self.proj_lm(obs[:, 4:idx_lm_end].view(batch_size, self.n, 2)) # (B, N, H)
        f_other = self.proj_other(obs[:, idx_lm_end:idx_other_end].view(batch_size, self.n-1, 2)) # (B, N-1, H)
        
        # --- 步骤 2: 准备 ID 特征并进行拼接 (Concat) ---
        # 2.1 准备我的 ID
        my_id_v = self.agent_id_emb(agent_id.long()).view(batch_size, 1, self.id_dim) # (B, 1, ID_D)
        q = torch.cat([f_self, my_id_v], dim=-1) # (B, 1, H + ID_D)
        
        # 2.2 准备地标 ID (所有 Batch 共享)
        lm_ids = torch.arange(self.n, device=device)
        lm_id_v = self.lm_id_emb(lm_ids).unsqueeze(0).expand(batch_size, -1, -1) # (B, N, ID_D)
        k_lm = torch.cat([f_lm, lm_id_v], dim=-1) # (B, N, H + ID_D)
        
        # 2.3 准备队友 ID
        # 这里的逻辑稍复杂：需要排除掉“我”的那个 ID
        # 简化处理：你可以直接用一个全 0 的 ID 占位，或者传入一个队友 ID 列表
        # 这里建议先用全 0 或一个通用的“队友类”ID
        other_id_v = torch.zeros(batch_size, self.n-1, self.id_dim, device=device)
        k_other = torch.cat([f_other, other_id_v], dim=-1) # (B, N-1, H + ID_D)

        # --- 步骤 3: 运算 ---
        c_lm, attn_weights_lm = self.attn_lm(q, k_lm, k_lm, average_attn_weights=False)
        # 保存到模块属性，方便外部读取
        try:
        ...
        if self.n > 1:
            ...
            if active_mask is not None:
                other_mask = ~(active_mask[:, 1:].bool())
                c_other, attn_weights_other = self.attn_other(q, k_other, k_other, key_padding_mask=other_mask, average_attn_weights=False)
            else:
                c_other, attn_weights_other = self.attn_other(q, k_other, k_other, average_attn_weights=False)
        else:
            c_other = torch.zeros_like(q)
        ...
        # --- 步骤 4: 融合输出 ---
        combined = torch.cat([q, c_lm, c_other], dim=-1).squeeze(1) # (B, 3 * total_dim)
...
class MAPPO:
    def __init__(self, obs_dim, action_dim, num_agents, 
        ...
        # pass id_dim for concatenated ID embedding
        self.actor = ParallelAttentionActor(num_agents, obs_dim, actor_hidden, ID_EMB_DIM, action_dim).to(device)
    ...
    def take_action(self, obs, explore=True, agent_id=None):
        ...
        logits = self.actor(state, agent_id=agent_id_tensor)
    ...
    def update(self, buffer, batch_size=64):
        ...
        with torch.no_grad():
            ...
            old_logits = self.actor(obs, agent_id=agent_ids.long().view(-1))
        ...
        for _ in range(self.epochs):
            ...
            for start in range(0, dataset_size, batch_size):
                ...
                logits = self.actor(mb_obs, agent_id=mb_agent_ids_vec)
...
def main():
            ...
            while env.agents:
                ...
                for agent_id, agent in enumerate(agents):
                    ...
                    if agent in obs_dict:
                        _, act = mappo.take_action(obs_dict[agent], explore=True, agent_id=agent_id)
                        actions[agent] = act
    ...
    try:
        ...
        while env.agents and step < max_test_steps:
            # 选择确定性动作（不探索）
            acts = {}
            for agent_id, agent in enumerate(test_agents):
                if agent in test_obs:
                    _, a = mappo.take_action(test_obs[agent], explore=False, agent_id=agent_id)
                    acts[agent] = a
