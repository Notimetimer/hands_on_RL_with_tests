...
epochs = 10  # 10
eps = 0.2
update_steps = 42  # 固定步长更新间隔

steps_limit = 350*update_steps
...
steps_collected = 0  # 记录当前收集的步数
total_steps = 0

episode_counter = 0
# tracking_test2.reset 返回 (obs, info)
state, _ = env.reset(train=True)
done = False
episode_return = 0
with tqdm(total=int(steps_limit), desc='Iteration') as step_bar: # num_episodes
    while total_steps<steps_limit:  # episode_counter < num_episodes and 
        ...
        if not done:
            ...
            # 2. 保存到全局缓冲区
            global_buffer['states'].append(state)
            global_buffer['actions'].append(action)
            global_buffer['next_states'].append(next_state)
            global_buffer['rewards'].append(reward + 0 * reward_plus)
            global_buffer['dones'].append(done)
            global_buffer['action_bounds'].append(action_bound)
            steps_collected += 1
            total_steps += 1
