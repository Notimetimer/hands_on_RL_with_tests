...
class Env:
    ...
    def get_cost(self, action):
        ...
        if self.position[0] > self.max_pos - trigger and action[0] > 0: # 假设边界区域为2个单位
            temp = action[0] * (self.position[0] - (self.max_pos - trigger))/10 # 离边界越近，惩罚越大
        # 惩罚靠近下边界还输出下的动作
        if self.position[0] < self.min_pos + trigger and action[0] < 0:
            temp = abs(action[0]) * ((self.min_pos + trigger) - self.position[0])/10 # 离边界越近，惩罚越大
        cost=temp
        
        return cost # softplus(cost)
...
class PPOLagCont:
    ...
    def __init__(self, state_dim, hidden_dim, action_dim, actor_lr, critic_lr, cost_critic_lr,
        ...
        # --- 修改开始 ---
        # self.lambda_cost 现在是一个简单的浮点数，而不是一个 requires_grad=True 的张量
        self.lambda_cost = init_val
        self.nu_lr = nu_lr
        # --- 修改结束 ---
    ...
    # --- 修改开始 ---
    # 新增 lambda 更新方法
    def update_lambda(self, mean_episodic_cost):
        """根据一段时间内的平均累积成本来更新lambda。"""
        # 这是标准的拉格朗日乘子更新规则（梯度上升）
        update_val = self.nu_lr * (mean_episodic_cost - self.target_cost)
        self.lambda_cost += update_val
        # 保持 lambda 在预设范围内
        self.lambda_cost = max(self.lambda_min, min(self.lambda_max, self.lambda_cost))
    # --- 修改结束 ---
    ...
    def update(self, transition_dict, adv_normed=False, clip_vf=False, clip_range=0.2):
        ...
        for _ in range(self.epochs):
            ...
            # --- 修改开始 ---
            # 使用 self.lambda_cost (一个浮点数) 而不是张量。
            # detach()确保成本项的梯度不会流向 lambda_cost，因为它现在在循环外更新。
...
agent = PPOLagCont(state_dim, hidden_dims, action_dim, actor_lr, critic_lr, cost_critic_lr,
                      ...
                      lambda_min=0.0, lambda_max=20.0, # 扩大lambda范围
                      nu_lr=5e-4) # 调低lambda学习率
# --- 修改结束 ---
...
# 为 lambda 更新做准备
episode_cost_list = []
lambda_update_interval = 10 # 每10个episode更新一次lambda
# --- 修改结束 ---
...
with tqdm(total=int(num_episodes), desc='Iteration') as pbar:  # 进度条
    for i_episode in range(int(num_episodes)):  # 每个1/10的训练轮次
        episode_return = 0
        # --- 修改开始 ---
        episode_cost = 0 # 初始化当前episode的累积成本
        # --- 修改结束 ---
        if clear_batch_flag:
        ...
        while not done:  # 每个训练回合
            max_action_bound = 3
            max_action = max_action_bound 
            min_action = -max_action_bound 
            ...
            # --- 修改开始 ---
            episode_cost += cost # 累积当前episode的成本
            # --- 修改结束 ---
        ...
        # --- 修改开始 ---
        episode_cost_list.append(episode_cost) # 记录本episode的总成本
        
        # 检查是否达到了更新lambda的条件
        if (i_episode + 1) % lambda_update_interval == 0 and episode_cost_list:
            mean_cost = np.mean(episode_cost_list)
            agent.update_lambda(mean_cost)
            episode_cost_list = [] # 重置列表
        # --- 修改结束 ---

        if 1: # len(transition_dict['dones'])>20: # 逐batch更新
            # --- 修改开始 ---
            agent.update(transition_dict, adv_normed=1) # 启用优势归一化
            # --- 修改结束 ---
            clear_batch_flag=1
        if (i_episode + 1) >= 10:
        ...
        # 打印 lambda 值以供观察
        if (i_episode + 1) % 10 == 0:
            print(f"Episode {i_episode+1}, Lambda_cost: {agent.lambda_cost:.4f}")
