...
class Env:
    ...
    def step(self, move):
        ...
        reward = self.get_reward(self.position, move)
        # print(reward)
    ...
    def get_reward(self, state, move):
        ...
        # 安全动作奖励
        predict_state = state+move
        if self.min_pos+2 <= predict_state[0] <= self.max_pos-2:
            reward1 += 0
        else:
            reward1 += 0 - 1 * max(predict_state[0]-self.max_pos-2, self.min_pos+2-predict_state[0])
...
num_episodes = 2000 # fixme 如果不限制最小方差，500 的时候会梯度爆炸, 限制后1000 也会爆炸
