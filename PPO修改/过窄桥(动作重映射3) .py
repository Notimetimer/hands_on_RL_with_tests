'''
使用tanh作为第一层动作边界设置（不准用clip），u->an
scale时使用线性映射作为安全动作重映射（不准用clip）an->a
经验池中存储的是u，而环境执行的是a，更新网络时直接用u与高斯分布比较
'''
...
class Env:
    ...
    def step(self, move):
        self.position += move - 2.9 # + np.random.normal()
    ...
    def get_reward(self):
        if self.min_pos <= self.position[0] <= self.max_pos:
            optimal_pos = 0
            if self.position[0]<optimal_pos:
                reward1 = (self.position[0]-self.min_pos)/(optimal_pos-self.min_pos)
            else:
                reward1 = 1-(self.position[0]-optimal_pos)/(self.max_pos-optimal_pos)
            # reward1 = 1 - np.linalg.norm(self.position - optimal_pos) / 10
        else:
            reward1 = 0 # -3
        return reward1
...
class PPOContinuous:
    ...
    def update(self, transition_dict, action_bounds=None):
        ...
        u_s = torch.tensor(transition_dict['actions'], dtype=torch.float).to(self.device)
...
num_episodes = 500 # fixme 如果不限制最小方差，500 的时候会梯度爆炸, 限制后1000 也会爆炸
