...
class Env:
    ...
    def get_cost(self, action):
        ...
        # 惩罚对边界的靠近程度
        cost = np.linalg.norm(self.position - (self.max_pos + self.min_pos) / 2) / 10

        # trigger = 3
        # # 惩罚靠近上边界还输出上的动作
        # if self.position[0] > self.max_pos - trigger and action[0] > 0: # 假设边界区域为2个单位
        #     cost += 1/3/trigger * action[0] * (self.position[0] - (self.max_pos - trigger)) # 离边界越近，惩罚越大
        # # 惩罚靠近下边界还输出下的动作
        # if self.position[0] < self.min_pos + trigger and action[0] < 0:
        #     cost += 1/3/trigger * abs(action[0]) * ((self.min_pos + trigger) - self.position[0]) # 离边界越近，惩罚越大
...
class PPOLagCont:
    ...
    def __init__(self, state_dim, hidden_dim, action_dim, actor_lr, critic_lr, cost_critic_lr,
                 ...
                 critic_max_grad=2, actor_max_grad=2,
                 nu_lr=0.01, lambda_cost_init=0.0, target_cost=0.0): # PPO-Lagrangian 参数
        ...
        # PPO-Lagrangian 参数
        self.lambda_cost = torch.tensor(lambda_cost_init, dtype=torch.float, device=device, requires_grad=True)
    ...
    def update(self, transition_dict, adv_normed=False):
        ...
        # ###  test
        # # ---- 调试输出（临时）: 检查 cost 通路各项是否为常数/非零 ----
        # print("DBG costs mean/std:", float(costs.mean().item()), float(costs.std().item()))
        # with torch.no_grad():
        #     cc_states_mean = float(self.cost_critic(states).mean().item())
        #     cc_next_mean = float(self.cost_critic(next_states).mean().item())
        # print("DBG cost_critic(states).mean(), cost_critic(next).mean():", cc_states_mean, cc_next_mean)
        # print("DBG cost_td_target mean/std:", float(cost_td_target.mean().item()), float(cost_td_target.std().item()))
        # print("DBG cost_advantage mean/std:", float(cost_advantage.mean().item()), float(cost_advantage.std().item()))
        # print("DBG lambda_cost before:", float(self.lambda_cost.item()))
        # # ------------------------------------------------------------

        # 优势归一化（目前只发现了阻碍）
        if adv_normed: # 修改项，detach()
        ...
        for _ in range(self.epochs):
            ...
            # 用.data，不用+=是为了防止破坏计算图
            self.lambda_cost.data.clamp_(min=0.0, max=10.0) # lambda_cost 必须是非负的
