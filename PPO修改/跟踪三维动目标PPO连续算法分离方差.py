...
class PPOContinuous:
    ...
    def take_action(self, state, action_bound=2.0, explore=True):
        ...
        mu = self.actor(state, action_bound=action_bound)
        sigma = action_bound*0.3 # 人工指定方差
        if not explore:
            action = mu
    ...
    def update(self, transition_dict):
        ...
        mu = self.actor(states, action_bound=action_bound)  # 均值、方差
        std = torch.tensor(action_bound*0.3) # 人工指定方差
        ...
        for _ in range(self.epochs):
            mu = self.actor(states, action_bound=action_bound)
            std = torch.tensor(action_bound*0.3) # 人工指定方差
            critic_values = self.critic(states)
