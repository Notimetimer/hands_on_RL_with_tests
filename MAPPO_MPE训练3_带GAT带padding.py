...
class GATBlock(nn.Module):
    ...
    Uses one or multiple internal GATLayerWithEdgeMask instances (heads).
    """
    def __init__(self, in_dim, out_dim, n_heads=1, dropout=0.0, alpha=0.2, concat=True):
        ...
        if n_heads == 1:
            self.heads = nn.ModuleList([GATLayerWithEdgeMask(in_dim, out_dim, dropout=0.0, alpha=alpha, concat=concat)])
            self.merge_linear = None
        else:
            head_dim = out_dim // n_heads
            self.heads = nn.ModuleList([GATLayerWithEdgeMask(in_dim, head_dim, dropout=0.0, alpha=alpha, concat=concat) for _ in range(n_heads)])
